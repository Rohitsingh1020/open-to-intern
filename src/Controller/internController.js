...
const Intern = async function (req, res) {
    try {
        ...
        if (!data) return res.status(400).send({ status:false ,message:'The request is not valid.'})
        let collegeDetail = await collegeModel.findById(collegeId)
        if (!collegeDetail) return res.status(404).send({status:false, message:'The request is not valid college id not found'})
